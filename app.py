...
st.markdown("Aplikasi ini untuk menganalisis sentimen dari satu atau lebih file ulasan aplikasi marketplace.")
...
if uploaded_files:
    ...
    for uploaded_file in uploaded_files:
        ...
        # Preprocessing
        with st.spinner('Tahap preproses data (1/3)...'):
        ...
        # TF-IDF
        with st.spinner('Tahap ekstraksi fitur (2/3)...'):
        ...
        # Klasifikasi MKNN
        with st.spinner('Tahap klasifikasi data (3/3)...'):
            k = 5
            alpha = 0.5
        ...
        positif_count = pred_counts.get('positif', 0)
        netral_count = pred_counts.get('netral', 0)
        negatif_count = pred_counts.get('negatif', 0)

        persentase_positif = (positif_count / total) * 100 if total > 0 else 0
        persentase_netral = (netral_count / total) * 100 if total > 0 else 0
        persentase_negatif = (negatif_count / total) * 100 if total > 0 else 0
    ...
    if sort_by_sentimen:
        file_results = sorted(
            file_results,
            key=lambda x: (
                round(x[3], 4),      # persentase_positif
                round(x[4], 4),      # persentase_netral
                -round(x[5], 4)      # persentase_negatif
            ),
            reverse=True
        )
    ...
    for idx, (filename, df_test, pred_counts,
        ...
        with display_area:
            ...
            with st.expander("Lihat Hasil Klasifikasi Lengkap"):
